...
Split training data into training / validation set for parameter tuning.
...
We also specify that we wish to save 
...
We desire the pentalty that gives us the simplest model, with the best accuracy.  Because of that, we select the 8th smallest penalty
...
Now that we've found the best parameter set, we need to apply it to the entire training dataset.

We made one tweek to our model specification.
...
We update our workflow to have the best parameter set.
